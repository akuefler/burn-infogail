...
nruns = 10 # training: 10, valid: 5
#nruns = 40
start_delta_freq = 75. # originally 75.
start_delta_s = collect(10.0:start_delta_freq:800.0)
ncars = length(start_roadinds) * length(start_delta_s)
#nframes_per_run = ceil(Int, 15.0 / NGSIM_TIMESTEP) # 30 seconds of driving
nframes_per_run = ceil(Int, 100.0 / NGSIM_TIMESTEP) # 30 seconds of driving
...
const N_FEATURES = 51
const MIX_CLASSES = true

const TRAIN_OR_VALID = "TRAIN"
...
for run in 1:nruns
    ...
    scene, carcolors, car_class = gen_start_state!(scene, context,
                                                   start_delta_s, models,
                                                   MIX_CLASSES)
    ...
    open(io->write(io, trajdata),
         @sprintf("../2d_drive_data/trajdata/trajdata_ncars%d_%s_%s%d.txt",
                  ncars, data_name, TRAIN_OR_VALID, run), "w")
    s = ("../2d_drive_data/car_classes/car_classes_ncars" * string(ncars) * "_" *
         data_name * "_" * TRAIN_OR_VALID * string(run) * ".jld")
    JLD.save(s, "classes", car_class)

    # extract features
    @printf "number veh defs: %d \n" length(keys(trajdata.vehdefs))
    for id in keys(trajdata.vehdefs)
        ...
        for frame in 1 : nframes_per_run
            ...
            #Auto2D.pull_metrics!(metrics, rec, trajdata.roadway, vehicle_index)
...
#outfile = "../2d_drive_data/data_distinct_drivers.jld"
#outfile = "../2d_drive_data/data_aggressive.jld"
outfile = @sprintf("../2d_drive_data/data/data_ncars%d_%s_%s.jld", ncars, data_name, TRAIN_OR_VALID)
#JLD.save(outfile, "features", featureset, "metrics", metricset, "run", runs, "targets", targetset, "intervals", interval_starts, "timestep", NGSIM_TIMESTEP, "classes", class_set)

JLD.save(outfile, "features", featureset, "run", runs, "targets", targetset, "intervals", interval_starts, "timestep", NGSIM_TIMESTEP, "classes", class_set)
