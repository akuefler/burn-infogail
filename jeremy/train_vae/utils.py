...
def save_h5(args, net, data_loader):
    ...
    with tf.Session() as sess:
        ...
        with h5py.File('/home/alex/stanford_dev/burn-infogail/data/baselines/encoder.h5','w') as f:
        ...
        with h5py.File('/home/alex/stanford_dev/burn-infogail/data/baselines/policy_vae.h5',
                'w') as f:
...
def latent_viz_pure(args, net, e, sess, data_loader):
    ...
    next_batch_dict = {'passive': data_loader.next_batch_pass, 'medium1':
            data_loader.next_batch_med1, 'medium2': data_loader.next_batch_med2, 'aggressive': data_loader.next_batch_agg}
...
def gen_samples_mixed(args, net, sess, data_loader):
    ...
    full_sample_agg = 0.0
    full_sample = {0.0:full_sample_pass, 1.0:full_sample_med2, 2.0:full_sample_med1, 3.0:full_sample_agg}
    ...
    for b in xrange(data_loader.n_batches_train):
        ...
        for i in xrange(len(z_samples)):
            class_train = c[i, 0]
            if type(full_sample[class_train]) is float:
                full_sample[class_train] = z_samples[i]
            else:
                full_sample[class_train] = np.vstack((full_sample[class_train], z_samples[i]))
...
def latent_viz_mixed(args, net, e, sess, data_loader):
    ...
    # Select random subset of values in full set of samples
    full_sample_pass = full_sample[0.0]
    full_sample_med2 = full_sample[1.0]
    full_sample_med1 = full_sample[2.0]
    full_sample_agg = full_sample[3.0]
    ...
    plt.legend(loc='upper right', numpoints=1)
    plt.grid()
    image_dir = ["./images_single", "./images_mix"][args.train_mix]
    #plt.savefig('./images_mixed/latent_viz_'+str(e)+'.png')
    plt.savefig(image_dir+"/latent_viz_"+str(e)+".png")
