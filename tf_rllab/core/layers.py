...
class LatentLayer(Layer):
    ...
    def get_output_for(self, input, **kwargs):
        ...
        latent = Z_mu + Z_sig * tf.random_normal(tf.shape(Z_mu))
        return latent
...
class ElemwiseMulLayer(MergeLayer):
    def __init__(self, incomings, **kwargs):
        super(ElemwiseMulLayer, self).__init__(incomings, **kwargs)

    def get_output_for(self, inputs, **kwargs):
        return functools.reduce(tf.mul, inputs)
